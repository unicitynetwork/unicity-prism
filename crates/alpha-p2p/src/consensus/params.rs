...
pub struct Params {
    ...
    /// The block height which RandomX becomes enforced (non-RandomX blocks are rejected).
    pub randomx_enforcement_height: u32,
    /// The multiplier for RandomX difficulty adjustment when SHA256D switches to RandomX.
    pub randomx_difficulty_multiplier: u32,
    /// Whether the PoW algorithm is RandomX (true) or SHA256D (false).
    pub pow_randomx: bool,
    /// The duration of each RandomX epoch, in blocks. If 0, then there are no epochs.
    ...
    /// Minimum blocks including miner confirmation of the total of 2016 blocks in a retargeting
    /// period, (nPowTargetTimespan / nPowTargetSpacing) which is also used for BIP9 deployments.
