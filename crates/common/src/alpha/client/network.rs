...
pub enum NetworkError {
    ...
    Consensus(#[from] EncodeDecodeError),
}
...
impl Decodable for NetworkMessageHeader {
    fn consensus_decode<R: Read + ?Sized>(reader: &mut R) -> Result<Self, EncodeDecodeError> {
        ...
        let command_bytes_slice = command_bytes
            .get(..end_pos)
            .ok_or_else(|| EncodeDecodeError::ParseFailed("Invalid command string slice"))?;
        let command_str = std::str::from_utf8(command_bytes_slice)
            .map_err(|_| EncodeDecodeError::ParseFailed("Invalid command string"))?;

        let command = CommandString::try_from(command_str)
            .map_err(|_| EncodeDecodeError::ParseFailed("Invalid command string"))?;
