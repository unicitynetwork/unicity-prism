    ...
    let expanded = match &input.data {
        Data::Struct(data) => {
            ...
            let field_constructions: Vec<_> = fields
                ...
                .map(|field| {
                    if let Some(ident) = &field.ident {
                        ...
                        quote! {
                            #ident: bitcoin::consensus::Decodable::consensus_decode_from_finite_reader(reader)?
                        }
                    } else {
                        // Tuple fields: just the decoded value
                        quote! {
                            bitcoin::consensus::Decodable::consensus_decode_from_finite_reader(reader)?
                        }
                    }
                })
                .collect();
            ...
            let field_constructions_limited: Vec<_> = fields
                ...
                .map(|field| {
                    if let Some(ident) = &field.ident {
                        quote! {
                            #ident: bitcoin::consensus::Decodable::consensus_decode(&mut limited_reader)?
                        }
                    } else {
                        quote! {
                            bitcoin::consensus::Decodable::consensus_decode(&mut limited_reader)?
                        }
                    }
                })
                .collect();
            ...
            quote! {
                impl #impl_generics bitcoin::consensus::encode::Encodable for #name #ty_generics #where_clause {
                    #[inline]
                ...
                impl #impl_generics bitcoin::consensus::encode::Decodable for #name #ty_generics #where_clause {
                    #[inline]
                    ...
                    fn consensus_decode<R: bitcoin::io::Read + ?Sized>(
                        ...
                        use bitcoin::io::Read;
                        let mut limited_reader = reader.take(bitcoin::consensus::encode::MAX_VEC_SIZE as u64);
        ...
        Data::Enum(data) => {
            ...
            let decode_arms: Vec<_> = data.variants.iter().enumerate().map(|(i, variant)| {
                ...
                match &variant.fields {
                    Fields::Unnamed(fields) if fields.unnamed.len() == 1 => {
                        ...
                        quote! {
                            #discriminator => {
                                let msg = bitcoin::consensus::Decodable::consensus_decode_from_finite_reader(reader)?;
            ...
            quote! {
                ...
                impl #impl_generics Decodable for #name #ty_generics #where_clause {
                    fn consensus_decode_from_finite_reader<R: bitcoin::io::Read + ?Sized>(
                        reader: &mut R,
                    ) -> core::result::Result<Self, bitcoin::consensus::encode::Error> {
